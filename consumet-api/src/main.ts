...
(async () => {
  ...
  await fastify.register(books, { prefix: '/api/books' });
  await fastify.register(anime, { prefix: '/api/anime' });
  await fastify.register(manga, { prefix: '/api/manga' });
  //await fastify.register(comics, { prefix: '/comics' });
  await fastify.register(lightnovels, { prefix: '/api/light-novels' });
  await fastify.register(movies, { prefix: '/api/movies' });
  await fastify.register(meta, { prefix: '/api/meta' });
  await fastify.register(news, { prefix: '/api/news' });

  await fastify.register(Utils, { prefix: '/api/utils' });

  try {
    fastify.get('/api', (_, rp) => {
      rp.status(200).send(
